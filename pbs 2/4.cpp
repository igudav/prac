...
int main(int argc, char **argv)
{
    ...
    complex c{argv[ARG_CENTER]};
    double r;
    sscanf(argv[ARG_RADUIS], "%lf", &r);
    int n;
    sscanf(argv[ARG_N], "%d", &n);
    char **f = &argv[ARG_FUNC];
    double angle = 0.0;
    complex integral{};

    for (int i = 0; i < n; ++i) {
        complex z = c + complex{r * cos(angle), r * sin(angle)};
        complex dz{-sin(angle), cos(angle)};
        integral = integral + eval(f, z) * dz;
        angle += 2 * M_PI / n;
    }

    integral = integral * (2 * M_PI * r / n);
    char buf[BUF_SZ];
